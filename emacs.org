...
(use-package go-mode
  ...
  (async-let ((goroot (shell-command-to-string "go env GOROOT")))
...
_t_: run current _t_est   _b_: run current _b_enchmark   _c_: run coverage
...
"
       ...
       ("c" (go-test-current-coverage) :color blue))
